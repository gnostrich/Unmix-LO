...
| 9 | THOUGHTWORLD | do frozen *vision* models' deviations from a physics seed carry atomic world-structure? | **NOISE (negative)** | Against a gauge-fixing physics engine, two frozen vision encoders' deviations are near-full-rank (eff-rank 16.4/20) and *indistinguishable from the random-fragment control* — no atomic directed structure. |
| 10 | THOUGHTWORLD-2 | do *dynamics-trained* frozen models (LLM, video) carry it? | **NOISE (generalized, confound-controlled)** | Both NOISE: VideoMAE is full-rank engine-leakage (eff-rank 16.1); the LLM's apparent structure (eff-rank 13.1, readout R² 0.57) is **prompt-arithmetic** — a required velocity-confound control shows pure arithmetic on the handed numbers *dominates* it (eff-rank 9.1, R² 0.82). No frozen model carries world-structure once the confound is controlled. |
| 11 | xresolve | do two frozen models have *independent* ambiguities (B resolves A's aliasing)? | **COINCIDING (null)** | On 135k CIFAR different-class pairs, every direction (cross-modal ViT↔text, cross-arch ViT↔DINO↔ResNet) scores 0.12–0.29 vs 0.5 baseline — *below* random, so models alias the **same** pairs (positive A–B distance corr). Cross-model ambiguity-resolution is toy-bound; the single-model paraconsistency dissociation (+0.059) stays banked. |
| 12 | fdt_denoise | is cross-model disagreement an FDT-satisfying *fluctuation* (native denoising, real MZ noise floor)? | **MID → DOWNGRADE (null-leaning)** | Estimator STEP-0-validated (OU/FDT-holds 0.81 vs violating ≤0.17). Real pairs land MID (0.49–0.54), never near the FDT-holds regime; a covariance-matched-noise control already scores 0.43, so genuine generator content above noise is only ~0.06–0.11, and vision state-readout R² is negative. Fabrication guard passes (literal-random floors at 0.13), so the null is trustworthy: the MZ "noise floor" is a heuristic threshold, not native content/noise separation. |
