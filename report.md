...
| 10 | THOUGHTWORLD-2 | do *dynamics-trained* frozen models (LLM, video) carry it? | **NOISE (generalized)** | Both give NOISE: VideoMAE is full-rank engine-leakage (eff-rank 16.1); the LLM predicts physics (readout R² 0.57) and is the most-structured fragment tested (eff-rank 13.1 < floor, beats its control, directed-frac 0.45) but fails held-out coherence (R² 0.16 < 0.3). Negative generalizes across fragment types. |
